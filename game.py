...
# ================> Game Restart <=================
def gameRestart():
    global player_X,player_Y,numberOfVirus,killVirus,numberOfBullet,createVirusSize,countCreateVirus,canLive,toConfig,totalCash,isStart,listOfVirus,listOfBullet,listOfLives,listOfCash,listOfDiedVirus,enamyX
    canvas.delete("all")
    player_X = 150
    player_Y = 450
    enamyX = 1400
...
def moveSmallVirus(smallVirus):
    ...
    if isStart:
        canvas.after(50, lambda:moveSmallVirus(smallVirus))
...
def moveBullet(bullet):
    canvas.move(bullet, +25, 0)
    listOfBullet.append(bullet)
    if isStart:
        canvas.after(2, lambda:moveBullet(bullet))
...
def checkBulletMeetVirus():
    ...
    listOfBullet = []
    if isStart:
        canvas.after(15, lambda:checkBulletMeetVirus())
...
canvas.tag_bind("replay","<Button-1>", gameStart)
